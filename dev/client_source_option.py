...
class FTPListener:
	...
	def sniff_filter(self, pkt):
		return pkt.haslayer(IP) and pkt[IP].src==self.dst and pkt[IP].dst == self.src and pkt.haslayer(TCP) and pkt[TCP].dport == self.sport and pkt[TCP].sport == self.dport
...
class FTPPassive:
	...
	def __init__(self, src, dst, sport, dport):
		...
		self.verbose = False
		self.listener = FTPListener(self.src, self.dst, self.sport, self.dport)
...
class FTPClient:
	...
	def manage_passive(self, command):
		...
		passive = FTPPassive(self.src, self.dst, sport, self.passive_port)
		passive.handshake()
...
if __name__ == '__main__':
	...
	client = FTPClient(source_ip, dest_ip, source_port, dest_port)
	client.handshake()
